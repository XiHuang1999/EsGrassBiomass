...
def csv2shp(csv_path, shp_path, xx, yy, layerName):
    '''
    将csv文件转为shp
    :param csv_path: str, csv文件的路径名称，第一列为Lon 第二列为Lat;
    :param shp_path: str, shp文件的路径名称;
    :param xx: int, Lon
    :param yy: int, Lat
    :param layerName: str, 图层名称
    :return: None
    '''
    # 解决中文字符问题
    ...
    for i in range(len(csv_df)):
        ...
        # 设置几何信息部分
        # 利用经纬度创建点,X为经度,Y为纬度
        point.AddPoint(float(csv_df.iloc[i, xx]), float(csv_df.iloc[i, yy]))
