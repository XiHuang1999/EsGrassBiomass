...
import EsRaster,readConfig,RF_Algorithm_V2_RF5
...
if __name__=="__main__":
    ...
    exlFile = r"G:\1_BeiJingUP\AUGB\Data\20220629\ALL_SITES_Select(gDMm-2)_数据来源分类.xlsx"
    ...
    for yr in range(2000,2021):
        ...
        stcDf = EsRaster.SampleRaster_gdal3(staticPath, yrcsv, 'ID',LonFieldName=r'LON', LatFieldName=r'LAT')
    ...
    allyr.to_csv(r"G:\1_BeiJingUP\AUGB\Data\FigData\allyr_20230226.csv")
    ...
    allyr = pd.read_csv(r"G:\1_BeiJingUP\AUGB\Data\FigData\allyr_20230226.csv",index_col=0)
    ...
    Ycols = [i for i in allyr.columns if
             i not in ['AGB', 'LON', 'Parameters_LAT', 'Parameters','Slope','Aspect','Soil_PH_h2o','Year','CGrass','Field1']]#,'Soil_Clay.tif', 'Soil_CoarseSand.tif', 'Soil_FineSand.tif','Soil_OrganicMass.tif', 'Soil_PH', 'Soil_PowderedSand.tif', 'Parameters_cgrass']]  # 'Year', or Ycols = allyr[allyr.columns.difference(['A', 'B'])]
    ...
    allyr = allyr[(allyr[r'CGrass'] > 0)]  # & (allyr[r'CGrass'] > 0)
    ...
    algX = allyr[Ycols]
    print(Ycols)
    RF_Algorithm_V2_RF5.RFEstimate(algX, algY, [staticPath, dynamicParasD], 0.90)
    # # for i in range(1,24):
