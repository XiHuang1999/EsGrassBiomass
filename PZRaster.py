...
def SampleRaster_gdal3(tifList, ptShp, siteName, nirCellNum=1, Scope='',
    ...
    for tif, tif_i in zip(tifList, range(len(tifList))):
        ...
        if 'NDVI-' in os.path.basename(tif).upper():
            for i in range(len(station_list)):
                # LonLat to ColRow
                [x, y] = CoordSys.geo2imagexy_gdal3(dr, xValues[i], yValues[i])
                # if math.isnan(x):
                #     print('sdf')
                # 判断方框滤波
                if nirCellNum == 1:
                ...
                if Scope == '':
                    ...
                    print('EsRaster Error！ Shp ID in ' + str(i))
                del dt
        elif 'NDVI' in os.path.basename(tif).upper():
            for i in range(len(station_list)):
                # LonLat to ColRow
            ...
            if 'BRT' in os.path.basename(tif).upper():
                print()
            for i in range(len(station_list)):
                # LonLat to ColRow
                [x, y] = CoordSys.lonlat2imagexy(dr, yValues[i], xValues[i])  # Gdal3后版本 维在前经在后
...
def ResampleLargeRaster(tif=r'G:\1_BeiJingUP\AUGB\Data\20220629\NDVI\NDVImax2015.tif', blockRow=10000, blockCol=10000,
                        poolCount=20):
    '''
    重采样大型栅格数据
    :param tif: str,栅格
...
def GdalReprojectImage(srcFile, outFile, refFile=r'G:\1_BeiJingUP\AUGB\Data\20220629\TAVG\TAVG_2000001.tif',
                       resampleFactor=1, sampleMethod='average'):
    """
