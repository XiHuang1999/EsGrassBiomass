...
def RFEstimate(X,
    ...
    import os
    os.environ['PROJ_LIB'] = r'J:\代码\PyCharmPythonFiles\PythonFiles_IGSNRR5\Lib\site-packages\osgeo\data\proj' #r'I:\代码\PyCharmPythonFiles\PythonFiles_IGSNRR5\Lib\site-packages\osgeo\data\proj' #r"E:\Anaconda\Anaconda\Lib\site-packages\osgeo\data\proj"
    ...
    # outexcel.to_excel(r'J:\1_BeiJingUP\AUGB\Data\20220629\Results\RF_AGB_2024\DataTable\GridCV.xlsx')



    # l1 = list(range(1450,1501,1)) #64 list(outexcel2.iloc[0:13,4])
    # # l1 = list(range(10,130,1)) #64 list(outexcel2.iloc[0:80,4])
    # l1 = np.array_split(l1, 50)
    # l2 = l1
    # l3 = l1
    # l4 = l1
    # l5 = l1
    # # temp = pd.DataFrame(l1)
    ...
    l1 = [21, 23, 22, 53, 55, 56, 52, 60, 54, 57, 61, 63, 62, 24, 71, 51] + \
         [1448, 1447, 1420, 1421, 1423, 1422, 1430, 1424, 1446, 1449, 1425, 1445, 1437, 1443, 1431, 1444, 1442, 1432, 1429, 1427, 1440, 1426, 1434, 1419, 1433, 1428, 1439, 1441, 1436, 1418, 1435, 1416, 1438, 1400, 1417, 1412, 1414, 1411, 1404, 1405, 1406, 1402, 1415, 1407, 1413, 1401, 1410, 1408, 1403, 1409]
    l2 = list(range(1,35,1))
    l3 = list(range(4,35,1))
    temp = pd.DataFrame(list(product(l1, l2, l3)))
    l1 = np.array_split(temp.iloc[:, 0], 10000)
    l2 = np.array_split(temp.iloc[:, 1], 10000)
    l3 = np.array_split(temp.iloc[:, 2], 10000)
    l4 = l3
    l5 = l3

    # outexcel = load_variavle(r'J:\1_BeiJingUP\AUGB\Data\20220629\Results\RF_AGB_2024\DataTable\outexcel_GridCV---n_estimators-max_depth-max_features.pickle')
    # outexcel1 = pd.DataFrame(outexcel)
    # outexcel2 = outexcel1.sort_values(by=1, ascending=False)
    # outexcel2 = outexcel2.iloc[0:10, 3:6]
    # dt = sum(  [(list(outexcel2.iloc[i,:])*90*60) for i in range(0,outexcel2.shape[0])]  , [])
    # dt = pd.DataFrame(np.array(dt).reshape([-1, 3]))
    # l4 = [i for i in range(1,91,1)]
    # l5 = [i for i in range(2,62,1)]
    # temp = pd.DataFrame(list(product(l4, l5)))
    # temp = pd.DataFrame(temp.values.tolist()*10)
    # l1 = np.array_split(dt.iloc[:, 0], 10000)
    # l2 = np.array_split(dt.iloc[:, 1], 10000)
    # l3 = np.array_split(dt.iloc[:, 2], 10000)
    # l4 = np.array_split(temp.iloc[:, 0], 10000)
    # l5 = np.array_split(temp.iloc[:, 1], 10000)

    X_train_ = [X_train for i in range(10000)]
    y_train_ = [y_train for i in range(10000)]
    X_test_ = [X_test for i in range(10000)]
    y_test_ = [y_test for i in range(10000)]
    X_ = [X for i in range(10000)]
    Y_ = [Y for i in range(10000)]
    kwgs = list(zip(X_train_, y_train_, X_test_, y_test_, X_, Y_, l1, l2, l3, l4, l5))  #

    outexcel = run_imap_mp(generate_mulcpu_vars_Est, kwgs, num_processes=15, is_tqdm=True)
    outexcel = sum(outexcel,[])
    save_variable(outexcel, r'J:\1_BeiJingUP\AUGB\Data\20220629\Results\RF_AGB_2024_5\DataTable\outexcel_GridCV---n_estimators-max_depth-max_features.pickle') #----min_samples_split-min_samples_leaf
    # outexcel = load_variavle(r'J:\1_BeiJingUP\AUGB\Data\20220629\Results\RF_AGB_2024_5\DataTable\outexcel_GridCV---n_estimators1400-1450.pickle')
    outexcel1 = pd.DataFrame(outexcel) # outexcel1.to_excel(r'J:\1_BeiJingUP\AUGB\Data\20220629\Results\RF_AGB_2024\DataTable\GridCV.xlsx')
    outexcel2 = outexcel1.sort_values(by=1, ascending=False)  # c = pd.concat([a, b], axis=0, join='outer')
    ...
    # RF_kwargs = {'n_estimators': 324, 'max_features': 3, 'bootstrap': True, "random_state":99,
    ...
    # # plt.savefig(r'J:\1_BeiJingUP\AUGB\Data\20220629\Results'+os.sep+r'PIC'+os.sep+'RF_results.png',dpi=500,bbox_inches='tight')#, transparent=True
    # # plt.savefig(r'J:\1_BeiJingUP\AUGB\Data\20220629\Results' + os.sep + r'PIC' + os.sep + 'RF_results_test2024-1.pdf',
    ...
    # # plt.legend()
    # # # plt.savefig(r'J:\1_BeiJingUP\AUGB\Data\20220629\Results'+os.sep+r'PIC'+os.sep+'RF4_results.png',dpi=500,bbox_inches='tight')#, transparent=True
    # # plt.show()
    #
    # # xy = np.vstack([y_train, outResults1])
    # # z = gaussian_kde(xy)(xy)
    ...
    #
    # # read and predict
    # para_Output[0]=[r'J:\1_BeiJingUP\AUGB\Data\20220629\Parameters\LAT_China1km.tif',
    #                 r'J:\1_BeiJingUP\AUGB\Data\20220629\Parameters\dem_china1km.tif',
    #                  r'J:\1_BeiJingUP\AUGB\Data\20220629\Soil\Clay.tif',
    #                  r'J:\1_BeiJingUP\AUGB\Data\20220629\Soil\CoarseSand.tif',
    #                  r'J:\1_BeiJingUP\AUGB\Data\20220629\Soil\FineSand.tif',
    #                  r'J:\1_BeiJingUP\AUGB\Data\20220629\Soil\OrganicMass.tif',
    #                 r"J:\1_BeiJingUP\AUGB\Data\20220629\Soil\PH_H2O.tif",
    #                  r'J:\1_BeiJingUP\AUGB\Data\20220629\Soil\PowderedSand.tif']
    ...
    #
    #     EsRaster.write_img(r"J:\1_BeiJingUP\AUGB\Data\20220629\Results\RF_AGB_2024\全区域"+os.sep+r'RF_AGB_'+str(yr)+r'.tif', im_proj, im_geotrans, outdat)
...
def generate_mulcpu_vars_Est(args):
    ...
    :return:
    '''
    return predict_Block_Est(args[0],args[1],args[2],args[3],args[4],args[5],args[6],args[7],args[8],args[9],args[10])

def predict_Block_Est(X_train, y_train, X_test, y_test, X, Y, i, ii, iii, iiii, iiiii):
    '''
    Predict by block
    :param est: obj, estimator
    :param dataX: ndarray, X var after np.split(a, 2, axis=1)
    ...
    '''
    li = []
    for ri,rii,riii,riiii,riiiii in zip(i,ii,iii,iiii,iiiii):
        ...
        #              'min_samples_leaf':ri, 'min_samples_split':rii  }  #, 'min_samples_split':rii , 'min_samples_leaf':ri

        # RF_kwargs = {'n_estimators': ri, 'bootstrap': True, "random_state": 99}  #, 'min_samples_split':rii
        RF_kwargs = {'n_estimators': ri, 'bootstrap': True, "random_state": 99, 'max_depth':rii, 'max_features': riii }
        # RF_kwargs = {'n_estimators': 102, 'bootstrap': True, "random_state": 99, 'max_depth':29, 'max_features': 4,
        #              'min_samples_leaf':ri, 'min_samples_split':rii  }
        # RF_kwargs = {'n_estimators': ri, 'bootstrap': True, "random_state": 99, 'max_depth':rii, 'max_features': riii,
        #              'min_samples_leaf':riiii, 'min_samples_split':riiiii  }
        ...
        li.append([linreg1.rvalue ** 2,linreg2.rvalue ** 2,linreg3.rvalue ** 2, ri, rii, riii, riiii, riiiii, score]) #
