...
import EsRaster,readConfig,RF_Algorithm_V2_RF5_2024
...
if __name__=="__main__":
    ...
    # # '''读取站点文件'''
    # # I:\1_BeiJingUP\CommonData\temp\Export_Output_2.shp
    ...
    allyr = pd.read_excel(r"G:\1_BeiJingUP\AUGB\Data\20220629\allyr_V2_Deleted_prov.xlsx",index_col=0)
    allyr = allyr[(allyr[r'Soil_Clay'] > -9999) & (allyr[r'CGrass'] < 60) & (allyr[r'AGB'] <= 1240)] #& (allyr[r'CGrass'] > 0)
    allyr = allyr.iloc[:,1:]
    ...
    Ycols = [i for i in allyr.columns if
             i not in ['AGB', 'ID', 'LON', 'Parameters','Slope','Aspect','Soil_PH_h2o','Year','CGrass','Field1','省代码', '省',
                       'Season1_TA', 'Season2_TA', 'Season1_PR', 'Season2_PR', 'Season1_SW', 'Season2_SW', 'Season1_FP', 'Season2_FP']]#,'Soil_Clay.tif', 'Soil_CoarseSand.tif', 'Soil_FineSand.tif','Soil_OrganicMass.tif', 'Soil_PH', 'Soil_PowderedSand.tif', 'Parameters_cgrass']]  # 'Year', or Ycols = allyr[allyr.columns.difference(['A', 'B'])]
    ...
    allyr = allyr[ ((allyr[r'省'] == r'西藏自治区')&(allyr[r'AGB'] < 80)) |  (allyr[r'省']!=r'西藏自治区') ]
    allyr = allyr[ ((allyr[r'省'] == r'青海省') & (allyr[r'AGB'] > 7)) | (allyr[r'省'] != r'青海省')]
    # a = allyr[allyr[r'省'] == r'青海省']
    # b = allyr[allyr[r'省'] == r'西藏自治区']
    # np.min(a.loc[:,'AGB'])
    # np.min(allyr.loc[:,'AGB'])


    allyr = allyr[ (allyr[r'CGrass'].isin([3,6,7,8,9]) & (allyr[r'AGB'] < 40)) | (~allyr[r'CGrass'].isin([3,6,7,8,9])) ]


    allyr = allyr.drop(columns=['省','省代码'])
    allyr = allyr.groupby(['LON','LAT','Year']).mean()
    allyr = allyr.reset_index(drop=False)
    # allyr = allyr.reset_index(drop=True)
    # deleteIndex = [  17,  505, 2193, 2202, 2541, 2665, 2760, 2762, 2917]
    # deleteIndex = list(dict.fromkeys(deleteIndex))
    # allyr = allyr.drop(deleteIndex)
    # allyr = allyr.reset_index(drop=True)
    # deleteIndex = [ 900, 2189, 2682, 3018]
    # deleteIndex = list(dict.fromkeys(deleteIndex))
    # allyr = allyr.drop(deleteIndex)
    ...
    algX = allyr[Ycols]
    print(Ycols)
    RF_Algorithm_V2_RF5_2024.RFEstimate(algX, algY, [staticPath, dynamicParasD], 0.90)
