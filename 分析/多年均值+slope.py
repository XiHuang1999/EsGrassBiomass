...
vmins = [  0, -60,    0, -180,    0, -260, 0, -0.2]
vmaxs = [500,  60, 1500,  180, 2500,  260, 1,  0.2]
...
txt = ['a','e','b','f','c','g','d','h']
cbLabel = ['{x:.0f}','{x:.0f}','{x:.0f}','{x:.0f}','{x:.0f}','{x:.0f}','{x:.1f}','{x:.1f}']
lbtitle = ['','Slope of ']
for axi,tifi,ii in zip(axlist,tif,range(len(axlist))):
    ...
    if ii == 1:
        ...
        bins = [-0.01, -0.005, 0, 0.005, 0.01, 0.2]
    else:
    ...
    if ii in [1,3,5,7]:
        ...
        cmaps = mpl.cm.get_cmap(cbar[ii%2], nbin)
    ...
    cb = fig.colorbar(imgtif,label=lbtitle[ii%2]+tifi.split(os.sep)[-1].split(r'_')[0].split('Mean')[0],\
