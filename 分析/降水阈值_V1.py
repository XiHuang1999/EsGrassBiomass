...
# region Parameters
# tif相关参数
tif = [r"G:\1_BeiJingUP\AUGB\Table\显著阈值\tif\sig_PRCP_Value_Proj.tif",
       r"G:\1_BeiJingUP\AUGB\Table\显著阈值\tif\sig_P_Value_Proj.tif",
       r"G:\1_BeiJingUP\AUGB\Table\显著阈值\tif\sig_R2_Value_Proj.tif",
       r"G:\1_BeiJingUP\AUGB\Table\显著阈值\tif\sig_Slope_Value_Proj_factor001.tif",
       ...
       r"G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\OtherSlp\GLASS_2000to2018_slope_Proj_Setnull.tif",
       r"G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\OtherSlp\GLOPEM-CEVSA_2000to2018_slope_Proj_Setnull.tif",
       r"G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\OtherSlp\MODIS_2000to2018_slope_Proj_Setnull.tif",
       r"G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\OtherSlp\MuSyQ_2000to2018_slope_Proj_Setnull.tif"
       ]
...
cbar = ['RdYlGn','Spectral','PuOr','summer'] #'Blues_r','viridis','Spectral_r'
# fig相关参数
row,col = 4,4
startLon = 72
endLon = 136
...
vmins = [  0,    0,    0, -300, -60, -180, -260, -0.2]     # [  0, -60,    0, -180,    0, -260, 0, -0.2]
vmaxs = [2000,   1,    1,  300,  60,  180,  260,  0.2]     # [500,  60, 1500,  180, 2500,  260, 1,  0.2]
...
cbLabel = ['{x:.0f}','{x:.2f}','{x:.1f}','{x:.1f}','{x:.3f}','{x:.3f}','{x:.3f}','{x:.3f}']
lbtitle = ['','Slope of ']
for axi,tifi,ii in zip(axlist,tif,range(len(axlist))):
    ...
    if ii in [0,1,2]:
        img_data[img_data >= vmaxs[ii]] = np.nan
        img_data[img_data <= vmins[ii]] = np.nan
    else:
    ...
    # elif ii == 6:
    #     # bins = [-100,-20,-10,0,10,20,100]
    #     bins = [-0.2, -0.01, -0.005, -0.001,0, 0.005, 0.01, 0.2]
    # elif ii == 7:
    #     bins = [-0.2, -0.01, -0.005, -0.001, 0, 0.005, 0.01, 0.2]
    else:
    ...
    if ii in [9]:#[4,5,6,7]:
        ...
        cmaps = mpl.cm.get_cmap(cbar[1], nbin)
    elif ii in [1]:
        cmaps = mpl.cm.get_cmap(cbar[3], nbin)
    elif ii in [2]:
        cmaps = mpl.cm.get_cmap(cbar[0], nbin)
    elif ii in [3]:
        cmaps = mpl.cm.get_cmap(cbar[2], nbin)
...
# plt.tight_layout()
plt.savefig(r'G:\1_BeiJingUP\AUGB\Table\显著阈值\Pic\降水阈值_'+(datetime.today().strftime("%y%m%d(%H%M%S)"))+'.pdf',dpi = 2500)
plt.show()
...
#        r"G:\1_BeiJingUP\AUGB\Data\SEM_2000_2013\Slope_2000_2013_2\Livestock\CGrass_Slope\Livestock_ha_2000to2018_slope_Proj.tif",
#        # r"G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\AnalysisBMA_Mean_Slp\TAVG0_2000to2018_slope_CGrass_Proj.tif",
#        r"G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\AnalysisBMA_Mean_Slp\PRCP0_2000to2018_slope_CGrass_Proj.tif",
#        r"G:\1_BeiJingUP\AUGB\Data\SEM_2000_2013\Slope_2000_2013_2\CO2\Slope\CO2_2000to2018_slope_Proj.tif",
#        r"G:\1_BeiJingUP\AUGB\Data\20220629\NDVI\PostProcess_CGrass_Slope\NDVI_2000to2018_slope_Proj1.tif"
#        # r'G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\AnalysisBMA_Mean_Slp\ANPP_2000to2018_slope_Proj.tif',
#        # r'G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\AnalysisBMA_Mean_Slp\BNPP_2000to2018_slope_Proj.tif',
#        # r'G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\AnalysisBMA_Mean_Slp\TNPP_2000to2018_slope_Proj.tif',
#        # r'G:\1_BeiJingUP\AUGB\Data\EveryModel_NPP_3_Nan\AnalysisBMA_Mean_Slp\fBNPP_2000to2018_slope_Proj.tif',
...
# '''绘图'''
# axlist = [ax01,ax02,ax03,ax04,ax05,ax06,ax07,ax08]
# vmins = [  0,    0,    0, 0, -0.3, -180, -260, -0.2]     # [  0, -60,    0, -180,    0, -260, 0, -0.2]
# vmaxs = [500, 1500, 2500, 1,  0.3,  180,  260,  0.2]     # [500,  60, 1500,  180, 2500,  260, 1,  0.2]
# HNlocx = [0.33,    0.33+0.475,  0.33, 0.33+0.475,
#           0.33,    0.33+0.475,  0.33, 0.33+0.475]
# HNlocy = [0.775,            0.775,              0.775-0.2488,  0.775-0.24833,
#           0.775-2*0.24833,  0.775-2*0.24833,  0.775-3*0.24833,    0.775-3*0.24833]
...
# texty = [159.9,159.9,71.5,71.5,-17.0,-17.0,-105.5,-105.5]
# txt = ['a','b','c','d','a','b','c','d'] #['a','e','b','f','c','g','d','h']
# cbLabel = ['{x:.0f}','{x:.0f}','{x:.0f}','{x:.1f}','{x:.1f}','{x:.0f}','{x:.1f}','{x:.3f}']
# lbtitle = ['','Slope of ']
# for axi,tifi,ii in zip(axlist,tif,range(len(axlist))):
#     print('GGG->')
#     # region Main Pic
...
#     img_data[img_data<=-9998] = np.nan
#     if ii == 4:
#         img_data[img_data>=vmaxs[ii]] = vmaxs[ii]-0.0001
#         img_data[img_data<=vmins[ii]] = vmins[ii]+0.0001
#     # SHP
#     QTP = cfeat.ShapelyFeature(Reader(QTPshp).geometries(),projPlc, edgecolor='k', facecolor='none')
#     CHN = cfeat.ShapelyFeature(Reader(CHNshp).geometries(),projPlc, edgecolor='k', facecolor='none')
#     NH = cfeat.ShapelyFeature(Reader(NHshp).geometries(),projPlc, edgecolor='k',facecolor='none')
...
#         # bins = [-5, -1, -0.5, -0.2, 0, 0.2, 0.5, 1, 5]
#     elif ii == 5:
#         # bins = [-100,-20,-10,0,10,20,100]
#         bins = [-500, -100, -30, 0, 30, 100, 500]
#     elif ii == 6:
#         # bins = [-100,-20,-10,0,10,20,100]
#         bins = [1.6, 1.8, 2.0, 2.2, 2.4, 2.6]
#     elif ii == 7:
#         bins = [-0.06, -0.01, -0.005, 0, 0.005, 0.01, 0.06]
#     else:
#         colorBarSectionsNum = cbNum[ii % 2]
#         bins = np.arange(vmins[ii], vmaxs[ii]+(vmaxs[ii]-vmins[ii])/colorBarSectionsNum, (vmaxs[ii]-vmins[ii])/colorBarSectionsNum)
#     print(bins)
...
#
# # plt.tight_layout()
# # plt.savefig(r'G:\1_BeiJingUP\AUGB\Pic\多变量1_'+(datetime.today().strftime("%y%m%d(%H%M%S)"))+'.pdf',dpi = 2500)
# plt.savefig(r'G:\1_BeiJingUP\AUGB\Pic\多变量2_'+(datetime.today().strftime("%y%m%d(%H%M%S)"))+'.pdf',dpi = 2500)
# plt.show()
