...
###=================================='''图样式二'''================================================
gs = gridspec.GridSpec(3,1,height_ratios=[1,2,1],hspace=0.2)
fig = plt.figure(figsize=(8,4))  # 创建Figure对象
...
for ax in fig.axes:
    ...
    lines.extend(axLine)
    labels.extend(axLabel)
fig.legend(lines, labels, loc='upper right', bbox_to_anchor=(1, 1))  # 图例的位置，bbox_to_anchor=(0.5, 0.92),
# plt.tight_layout()
plt.savefig(r'G:\1_BeiJingUP\AUGB\Pic\QTP均值年际变化_Style2_'+(datetime.now().strftime("%H-%M-%S"))+'.png',dpi = 600)
plt.show()
# # 断轴
# d = .85  #设置倾斜度
