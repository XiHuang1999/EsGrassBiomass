...
xlsx = r"G:\1_BeiJingUP\AUGB\Table\时间序列验证\NDVI\Mean.xlsx"#r"F:\ScienceResearchProject\AUGB\Table\七个草地\Mean.xlsx"
...
for i in range(0,len(var)):
    ...
    ax.plot(pltx, plty, '--', color='black', alpha=0.8, linewidth=1, label=r'Slope = ' + "%.3f" % linreg.slope  + "g/m\u00b2/yr \n P = " + "%.2f" % linreg.pvalue)    # color='#4169E1'
    ax.set_ylabel(var[i],labelpad=0) #X轴标签 ,fontproperties=font
    ax.set_xlabel("year",labelpad=0) #X轴标签

    ax.text(2000,ax.get_ylim()[0]+(ax.get_ylim()[1]-ax.get_ylim()[0])*7.3/10,
            r'('+chr(97+i)+')  Slope = ' + "%.2f" % linreg.slope + startP(linreg.pvalue)+'\n'+
            '       R$^{2}$ = %.2f' % (linreg.rvalue**2)
            ,linespacing=1.1) #" g/m\u00b2/yr"

plt.subplots_adjust(left=0.06, right=0.99, bottom=0.09, top=0.97, wspace=0.29, hspace=0.3)
...
plt.savefig(r'F:\ScienceResearchProject\AUGB\Pic'+os.sep+'七个fBNPP年际动态_'+(datetime.now().strftime("%H-%M-%S"))+'.svg'
