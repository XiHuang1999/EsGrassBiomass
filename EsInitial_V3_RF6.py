...
import EsRaster,readConfig,RF_Algorithm_V2_RF6
...
if __name__=="__main__":
    ...
    Ycols = [i for i in allyr.columns if
             i not in ['AGB', 'ID', 'LON', 'Parameters','Slope','Aspect','Soil_PH_h2o','CGrass','Field1','Year'
                       'Soil_Clay', 'Soil_Coars', 'Soil_FineS','Soil_Organ', 'Soil_PH_h2', 'Soil_Powde']]  # 'Year', or Ycols = allyr[allyr.columns.difference(['A', 'B'])]
    ...
    allyr = pd.read_csv(r"G:\1_BeiJingUP\AUGB\Data\20220629\RF5所使用的训练样本_去除西藏高值区域.csv")
    algY = allyr['AGB']
    algX = allyr[Ycols]
    print(Ycols)
    RF_Algorithm_V2_RF6.RFEstimate(algX, algY, [staticPath, dynamicParasD], 0.90)
    # # for i in range(1,24):
