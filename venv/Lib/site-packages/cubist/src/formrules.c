...
int Leaves, MaxDepth, NCond, Bestd;
...
/*   ---------  */
{
  ...
  Verbosity(1, fprintf(Of, "\n  Pruning rule"))
    ...
    Verbosity(1, fprintf(Of, "\n      Used   Err \tAbsent condition\n"))
      ...
      Verbosity(
          1, fprintf(Of, "   %7g%7.1f", rint(Total[d] * 10) / 10, PredErr[d]))
  ...
  if (NewRule(Cond, NCond, Deleted, Cover, Sum / SumWt, LoVal, HiVal,
    ...
    if (UNBIASED)
      RemoveBias(Rule[NRules], InitCoeffs);
  }
}
...
/*   ------------  */
{
  ...
  if (!Bestd) {
    ...
    ForEach(d, 0, NCond) { Total[d] = PredErr[d] = 0; }

    ForEach(i, 0, MaxCase) {
      if (!LocalNFail[i]) {
        UpdateCount(0, i, Total, PredErr);
        AddToList(&Fail0, i);
      } else if (LocalNFail[i] == 1) {
        d = SingleFail(i);
...
/*   ----------------  */
{
  ...
  FreeVector((void **)CondFailedBy, 0, MaxDepth + 1);
  CondFailedBy = Nil;
  Free(Stack);
  Stack = Nil;
  Free(Deleted);
