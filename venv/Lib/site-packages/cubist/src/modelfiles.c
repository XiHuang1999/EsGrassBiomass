...
char *Prop[] = {"null",   "id",      "att",    "elts",  "prec",   "globalmean",
                "floor",  "ceiling", "sample", "init",  "mean",   "sd",
                "mode",   "entries", "rules",  "cover", "loval",  "hival",
                "extrap", "insts",   "nn",     "maxd",  "esterr", "conds",
                "type",   "cut",     "result", "val",   "coeff",  "max",
                "min",    "redn"};

char PropName[20], *PropVal = Nil, *Unquoted;
int PropValSize = 0;
...
/*   ---------------  */
{
  ...
  now = localtime(&clock);
  now->tm_mon++;
  fprintf(Mf, "id=\"Cubist %s %d-%d%d-%d%d\"\n", RELEASE, now->tm_year + 1900,
          now->tm_mon / 10, now->tm_mon % 10, now->tm_mday / 10,
          now->tm_mday % 10);
  ...
  ForEach(Att, 1, MaxAtt) {
    ...
    if (Continuous(Att)) {
      fprintf(Mf, " mean=\"%.*g\" sd=\"%.*g\" min=\"%g\" max=\"%g\"\n", PREC,
              AttMean[Att], PREC, AttSD[Att], AttMinVal[Att], AttMaxVal[Att]);
    } else {
...
/*   -----------------  */
{
  ...
  ForEach(Att, 1, MaxAtt) {
    if (!StatBit(Att, DISCRETE) || MaxAttVal[Att] < 2)
      continue;

    AsciiOut("att=", AttName[Att]);
    AsciiOut(" elts=", AttValName[Att][2]); /* skip N/A */

    ForEach(v, 3, MaxAttVal[Att]) { AsciiOut(",", AttValName[Att][v]); }
    fprintf(Mf, "\n");
  }
}
...
/*   ---------  */
{
  ...
  ForEach(ri, 1, RS->SNRules) {
    R = RS->SRule[ri];
    fprintf(Mf,
            "conds=\"%d\" cover=\"%d\" mean=\"%.*f\" "
            "loval=\"%g\" hival=\"%g\" esterr=\"%.*f\"\n",
            R->Size, R->Cover, Precision + 1, R->Mean, R->LoVal, R->HiVal,
    ...
    ForEach(d, 1, R->Size) {
      ...
      case BrThresh:
        if (C->TestValue == 1) {
          ...
          fprintf(Mf, " cut=\"%.*g\" result=\"%s\"", PREC + 1, C->Cut,
                  (C->TestValue == 2 ? "<=" : ">"));
        }
        break;
...
/*   ---------  */
{
  ...
  while (true) {
    ...
    case IDP:
      ...
      if (sscanf(PropVal + strlen(PropVal) - 11, "%d-%d-%d\"", &Year, &Month,
                 &Day) == 3) {
        SetTSBase(Year);
      }
      break;
    ...
    case MODEP:
      Unquoted = RemoveQuotes(PropVal);
      Modal[Att] = Which(Unquoted, AttValName[Att], 1, MaxAttVal[Att]);
...
/*  -------------  */
{
  ...
  ForEach(m, 0, MEMBERS - 1) { Cttee[m] = InRules(); }
...
/*    ------  */
{
  ...
  ForEach(d, 1, R->Size) { R->Lhs[d] = InCondition(); }
  ...
  do {
    ...
    case VALP:
      if (Continuous(C->Tested)) {
        ...
        Unquoted = RemoveQuotes(PropVal);
        C->TestValue =
            Which(Unquoted, AttValName[C->Tested], 1, MaxAttVal[C->Tested]);
        if (!C->TestValue) {
          Error(MODELFILE, E_MFATTVAL, Unquoted);
        }
      }
